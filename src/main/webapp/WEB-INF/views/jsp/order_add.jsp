...
<meta charset="utf-8" />
<base href="${base}/" />
<title>核对订单信息-球鞋商城</title>
<jsp:include page="base.jsp" />
<script type="text/javascript">
	function finish() {
		var payType = $("#paymentBox input[name='payType']:checked")
				.val();
		var adddress = $("#address_often input[name='addressId']:checked").val();
		if (payType == "支付宝") {
			alert("很抱歉，本系统暂不支持该种支付方式！请选择预存款支付！");
		}else if(adddress==null){
			alert("请添加收货地址！");
		}else {
			$("#form1").submit();
		}
	...
	$(document).ready(function (){
		...
		tr.each(function () {
			price = $(this).html();
			console.log("hah "+price);
			$(this).html(Math.floor(price * 100) / 100);
		...
		$("#final_sum").html(Math.floor(($("#final_sum").html() )* 100) / 100);
					...
					<div class="cont">
						...
						<div class="wrap_box">
							...
							<div class="prompt_4 m_10" id='address_often'>
								<strong>常用收货地址</strong> <a href="./usercenter/address/list">添加收货地址</a>
								<ul class="addr_list">
								...
								<tbody id="deliveryFormTrBox">
									<tr>
										<th><label><input type="radio"
												name="deliveryType" paytype="0" alt="0.00" checked
												value="快递">快递</label></th>
										<td>直接由第三方物流公司配送 运费：￥20.00 &nbsp;&nbsp;</td>
									</tr>
									<tr>
										<th><label><input type="radio"
												name="deliveryType" paytype="0" alt="0.00"
...
<%--								</tr>--%>
						...
						<div class="wrap_box">
								...
								<tbody>
									...
									<c:forEach items="${orderDetails}" var="orderDetail" varStatus="s">
										...
										<tr>
											...
											<td class="bbb">￥<b class="red2" >${orderDetail.goods.price2*orderDetail.nums  }</b></td>
										</tr>
							...
							<c:choose>
								<c:when test="${user.role=='u'||user.role=='a'}">
									<input type="hidden" name="totalMoney" value="${totalMoney*0.9 }" />
								</c:when>
								<c:otherwise>
						...
						<div class="pink_box gray m_10">
							<table width="100%" class="form_table t_l">
								...
								<tr>
									<c:choose>
										<c:when test="${user.role=='u'||user.role=='a'}">
											<td class="t_r"><b class="price f14">会员价格为：
												<span class="red2">￥<b id='final_sum'>${totalMoney*0.9}</b></span>元</b></td>
										</c:when>
										<c:otherwise>
											<td class="t_r"><b class="price f14">应付总额：
												<span class="red2">￥<b id='final_sum' name="totalMoney">${totalMoney }</b></span>元</b></td>
										</c:otherwise>
									</c:choose>
