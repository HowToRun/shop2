...
<base href="${base}/" />
<title>我的订单_球鞋商城</title>
<jsp:include page="../base.jsp" />
<script type="text/javascript">
	$(function() {
		setSelectedClass("usercenter/order_list.jsp");
	});
	function pingjia(goodsid,orderid) {
		var remark = prompt("输入评价内容！");
		var score = 5;
		// location.href = "./comment/addaddComment?remark="+remark+"&statusNum=0&targetId="+id+"&score="+score;
		$.ajax({
			type: "POST",
			url: "./comment/addaddComment",
			data: {remark:remark, statusNum:0,targetId:goodsid,score:score,orderId:orderid},
			success: function(data){
				if(data.status==200){
					location.href = "./order/listByUser";
				}
			}
		});
	}
						...
						<c:forEach items="${ orders}" var="order">
							<tr>
								...
								<td><b class="orange">
									...
									<c:if test="${order.orderDetail.flag == 1}"><input type="button"  name="评价" value="评价" onclick="pingjia('${order.orderDetail.goodsId}','${order.id }')"/></c:if>
								</td>
							<tr>
						</c:forEach>
						<tfoot>
							<tr>
								<td colspan="6" class="t_l"><div class='pages_bar'>
										<a href=''>回到首页</a><a class='current_page' href=''>订单总数</a><span>共${num}条</span>
									</div></td>
							</tr>
						</tfoot>
					</table>
