...
<script type="text/javascript">
	$(function() {
        ...
        $("#email").on("blur",function(){
            ...
            if(!reg.test(email)){
                $("#emailmsg").html("请填写正确的邮箱格式");
                /*设置提交按钮不可用*/
                $("#submit").attr("disabled",true);
                $("#checkCodeBtn").attr("disabled",true );//设置按钮为禁用状态
            }else{
                $.get("./user/user_isexist/" + email+"/1", function(result) {
                    var user =result.data;
                    //	console.log(user);
                    if (user!= null) {
                        $("#emailmsg").html("该用邮箱可用");
                        $("#checkCodeBtn").attr("disabled",false );//设置按钮为禁用状态
                        $("#submit").attr("disabled",false);
                    } else {
                        $("#emailmsg").html("该邮箱未被注册");
                        /*设置提交按钮不可用*/
				$("#submit").attr("disabled",true);
				$("#checkCodeBtn").attr("disabled",true );//设置按钮为禁用状态
			}
        ...
        //点击事件设置button样式开始计时，发送验证码
         $("#checkCodeBtn").click(function(){
            // 设置button效果，开始计时
            curCount = count;
            $("#checkCodeBtn").attr("disabled",true );//设置按钮为禁用状态

            $("#checkCodeBtn").value=curCount + "秒后重获";//更改按钮文字
            InterValObj = window.setInterval(SetRemainTime, 1000); // 启动计时器timer处理函数，1秒执行一次
            //10秒之后弹出验证码
            setTimeout(sendEmailCode, 10000);
        ...
        function sendEmailCode(){
            ...
            if(Num.length>=6){//验证码发送成功
                $("#emailmsg").html("邮箱验证码已发到您的邮箱,请查收");
            }else {
                $("#emailmsg").html("短信验证码发送失败，请重新发送");
        ...
        function SetRemainTime() {
            if (curCount == 0) {//超时重新获取验证码
                ...
                $("#checkCodeBtn").attr("disabled",false);//移除禁用状态改为可用
                $("#checkCodeBtn").val("重获验证码");
            } else {
                curCount--;
               // console.log(curCount);
                $("#checkCodeBtn").val("("+curCount + ")秒后重获") ;
             }
	...
	<div class="brand_list container_2">
		...
		<div class="wrapper clearfix">
			<div class="wrap_box">
				...
				<p class="tips">
					<span class="gray f_r">已有优他商城帐号？请点<a class="orange bold"
						href="user/toLogin">这里</a>登录
					</span>忘记密码没关系，可以用邮箱找回哦> > >
				</p>
						...
						<table class="form_table f_l">
							...
							<tr>
								...
								<td><input class="gray" type="email" id="email"
									...
									<label id="emailmsg">填写有效邮箱</label>
								</td>
							</tr>
							<tr>
								<th>邮箱验证码：</th>
								<td><input style="width:126px;height: 30px; overflow:hidden;border: 1px solid #c4c4c4; margin: 0 5px 0 0; background: url(images/inputbg.gif) no-repeat" type="text" id="checkCode" name='checkCode' tabindex="2" "/></td>
								<td><input id="checkCodeBtn" type="button" style="height:32px;width:104px; margin-left:-263px; " value="点击发送验证码"  />
                                    <label id="codemsg">填写邮箱验证码</label></td>
							</tr>
							<tr>
								<th>新密码：</th>
