...
<script type="text/javascript">
	...
	$(document).ready(function (){
		var tr = $("#goodsList .aaa");
		var zongjia=0;
		var price;
		tr.each(function () {
			price = $(this).children().eq(4).html();
			$(this).children().eq(4).html(Math.floor(price * 100) / 100);
			price= Math.floor(price * 100) / 100;
				zongjia = zongjia + price;
				console.log(zongjia);
		});
		$("#sum_price").html(zongjia);
					...
					<tbody id="goodsList">
						...
						<c:forEach items="${cartList}" var="cart" varStatus="s">
							...
							<tr class="aaa">
								...
								<td>${cart.price}</td>
								<td><%--<input class="reduce" type="button" value="-" style="width: 15px;height: 15px">
									<input class="num" type="text" value="${cart.num}" style="width: 15px;height: 15px">
									<input class="add" type="button" value="+" style="width: 15px;height: 15px">--%>
									${cart.num}
								</td>
								<td class = "${s.index }">${cart.num * cart.price}</td>
								<td><!-- 修改操作 -->
