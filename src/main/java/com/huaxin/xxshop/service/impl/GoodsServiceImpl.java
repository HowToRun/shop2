...
import com.huaxin.xxshop.entity.SalesStatistics;
...
public class GoodsServiceImpl implements GoodsService {
	 ...
	 */
	@Override
	public List<Goods> findGoodsByWord(String word) {
		
		return goodsDao.findGoodsByWord(word);
	}

	@Override
	public PageBean<SalesStatistics> salesStatistics(Integer page, String startDate, String endDate,String category) {
		// 页面的大小
		int pageSize = 6;

		PageBean<SalesStatistics> pageBean = new PageBean<>();
		Map<String, Object> params = new HashMap<>();
		params.put("indexPage", (page - 1) * pageSize);
		params.put("pageSize", pageSize);
		...
		if (category!=null){
			String categoryId = categoryDao.getCategoryIDByName(category);
			categoryId = StringUtils.isNotBlank(categoryId)?categoryId:"";
			params.put("categoryId", categoryId);
		}

		List<SalesStatistics> data = goodsDao.salesStatistics(params);

		pageBean.setData(data);
		pageBean.setPage(page);
		pageBean.setPageSize(pageSize);
		pageBean.setActualPageSize(data.size());
		int totalNum = goodsDao.getStatisticssNum(params);
		pageBean.setTotalNum(totalNum);
		int totalPage = totalNum % pageSize == 0 ? totalNum / pageSize
				: totalNum / pageSize + 1;
		pageBean.setTotalPage(totalPage);
