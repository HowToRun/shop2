...
public class CommentAction {
    ...
    public ModelAndView listReCommentAboutGolistByUserods(String commentId, ModelAndView mv) {
    ...
    @RequestMapping("/addaddComment")
    @ResponseBody
    public SysResult addaddComment(HttpSession session, Comment comment,
                                String statusNum, ModelAndView mv,
                                String targetId, String score,String orderId) {
        comment.setId(XXShopUtil.getId());
        comment.setOwnerid(((User)session.getAttribute("user")).getId());
        comment.setTargetid(targetId);
        comment.setStatusnum(Integer.parseInt(statusNum));
